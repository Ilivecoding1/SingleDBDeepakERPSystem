...
{
    public partial class Unlock_Username : Form
    {
        SqlConnection conmmc = new SqlConnection(SqlconnMain.sbpayroll);
        public Unlock_Username()
        {
            InitializeComponent();
        }

        private void btnUnlockUsername_Click(object sender, EventArgs e)
        {
            if (txtEmpName.Text == "")
            {
            ...
            else
            {
                ...
                try
                {
                    ...
                    {
                        int userunlockrequest = 1;

                        if (conmmc.State == ConnectionState.Closed)
                            conmmc.Open();
                        SqlCommand cmd1 = new SqlCommand("insert into usernameunlock (username,userunlockrequestdate,userunlockrequest)values(@username,@datetime,@requestpass)", conmmc);
                        cmd1.CommandType = CommandType.Text;
                        cmd1.Parameters.AddWithValue("@username", txtEmpName.Text);
                        cmd1.Parameters.AddWithValue("@datetime", DateTime.Now);
                        cmd1.Parameters.AddWithValue("@requestpass", userunlockrequest);
                        cmd1.ExecuteNonQuery();
                        MessageBox.Show("Dear User you have sucessfully raised a request to unlock your username., You will get unlocked user information on your email shortly.", "Successful", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        try
                        {
                            //int u_pwd = 0;
                            ...
                            code.Execute(@"insert into usr_log (log_date,sys_name,sys_ip,usr_name,desc1)values(GETDATE(),'" + pcname + "/" + pcusername + "','" + ipaddress + "','" + txtEmpName.Text + "','Clicked on unlock username button and sent request for unlock username request for " + txtEmpName.Text + "user.')");
                        }
